/**
 * Script de migration de production.
 * S'exécute au démarrage du container avant NestJS.
 * Utilise $executeRawUnsafe pour éviter les limitations de prisma migrate.
 */
...
async function run() {
  ...
  const steps = [
    ...
    // Nouveaux types enum (ignorés si déjà existants)
    `DO $$ BEGIN
    ...
    // Restaurant — cuisine + restaurantType (colonnes enum — ajoutées après les types)
    `DO $$ BEGIN
    ...
    // Foreign keys (toutes IF NOT EXISTS via DO)
    `DO $$ BEGIN
  ...
  for (const sql of steps) {
    try {
      ...
      if (!e.message.includes('already exists') && !e.message.includes('duplicate')) {
        console.error(`[MIGRATE] WARN: ${e.message.substring(0, 120)}`);
        fail++;
      } else {
        ok++;
      }
